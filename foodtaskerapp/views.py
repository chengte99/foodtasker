...
def restaurant_report(request):
    #Calculate revenue and number of order in week
...
#Practices Challenge 1 & 2
@login_required(login_url='/restaurant/sign-in/')
def restaurant_customer(request, restaurant_id):
    orders = Order.objects.filter(restaurant = request.user.restaurant).order_by("-id")
...
#Practices Challenge 1 & 2
@login_required(login_url='/restaurant/sign-in/')
def restaurant_driver(request, restaurant_id):
    orders = Order.objects.filter(restaurant = request.user.restaurant, status = Order.DELIVERED).order_by("-id")
